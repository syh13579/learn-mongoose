...
app.use((req, res, next) => {
    //요청이 들어온 HTTP 메서드와 URL을 이용 -> 해당하는 라우터가 없음을 나타내는 에러 생성
...
app.use((err, req, res, next) => {
    ...
    res.render('error'); // 에러 페이지 렌덜이 -> 클라이언트에게 전달
});

// 특정 포트에서 서버 
app.listen(app.get('port'), () => {
    // 서버가 시작되었음을 콘솔에 출력
    console.log(app.getv('port'), '번 포트에서 대기 중')
});